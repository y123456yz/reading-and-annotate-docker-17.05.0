...
static void stat2proc(const char* S, proc_t *restrict P) { // proc/pid/stat文件内容存入p
    unsigned num;
    char* tmp;
...
ENTER(0x160);
    ...
    num = sscanf(S,
       ...
       &P->flags, &P->min_flt, &P->cmin_flt, &P->maj_flt, &P->cmaj_flt, //10
...
// room to spare.    读取/proc/pid/xxx相关的文件内容
...
static int listed_nextpid(PROCTAB *restrict const PT, proc_t *restrict const p) {
  ...
  printf("yang test ... listed_nextpid, path:%s, tgid:%d, p->tgid:%d, p->tid:%d\r\n", path, tgid, p->tgid, p->tid);
  return tgid;
}
 ...
 */ //读取PT->pids 进程对应的  /proc/PT->pids/对应的文件
...
//  }
  ...
  for(;;){
    ...
    ret = PT->reader(PT,p); //simple_readproc
    if(ret) return ret;
  }
...
proc_t* readeither (PROCTAB *restrict const PT, proc_t *restrict x) { //procs_refresh中执行
